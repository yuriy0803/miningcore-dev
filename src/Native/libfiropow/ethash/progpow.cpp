...
hash256 hash_final(const hash256& seed_hash, const hash256 *mix_hash_ptr) noexcept
{
    ...
    ethash_keccakf800(state);
    hash256 final_hash;
    std::memcpy(&final_hash, &state[0], sizeof(hash256));
    return final_hash;
}
...
{
    const hash256 seed_hash = hash_seed((const hash256 *)&header_hash, nonce);
    const uint64_t seed = seed_hash.word64s[0];
    const hash256 computed_mix_hash = hash_mix(context, block_number, seed, calculate_dataset_item_2048);
    const hash256 computed_final_hash = hash_final(seed_hash, (const hash256 *)&computed_mix_hash);
    
    /* mod start */
    if (!is_less_or_equal(computed_final_hash, boundary1)) {
		//if(boundary1 == boundary2) {
        if(is_equal(boundary1, boundary2)) {
		  *retcode = 1;
          return {computed_final_hash, mix_hash};
		}
	    else {
			if (!is_less_or_equal(computed_final_hash, boundary2)) {
				 *retcode = 1;
				 return {computed_final_hash, mix_hash};
			}
		}
	...
	/* mod end */
	*retcode = 0;
    return {computed_final_hash, computed_mix_hash};
}
    ...
    uint64_t nonce) noexcept
{
    ...
    const hash256 seed_hash = hash_seed((const hash256 *)&header_hash, nonce);
    const uint64_t seed = seed_hash.word64s[0];
    const hash256 computed_mix_hash = hash_mix(context, block_number, seed, lazy_lookup);
    const hash256 computed_final_hash = hash_final(seed_hash, (const hash256 *)&computed_mix_hash);
    ...
    const hash256& mix_hash, uint64_t nonce, const hash256& boundary) noexcept
{
    const hash256 seed_hash = hash_seed((const hash256 *)&header_hash, nonce);
    const uint64_t seed = seed_hash.word64s[0];
    const hash256 expected_mix_hash = hash_mix(context, block_number, seed, calculate_dataset_item_2048);
    const hash256 computed_final_hash = hash_final(seed_hash, (const hash256 *)&expected_mix_hash);


    if (!is_less_or_equal(computed_final_hash, boundary)) {
        return false;
	}
