...
#define XOR_8(A, B, C, D, F, G, H, I) crypto::xor_pod(XOR_4(A, B, C, D), XOR_4(F, G, H, I))
  ...
  int cryptonote_keccak_generic(const uint8_t *in, size_t inlen, uint8_t *md, size_t mdlen)
  {
    state_t_m st;
    uint8_t temp[144];
    size_t i, rsiz, rsizw;
    ...
    for ( ; inlen >= rsiz; inlen -= rsiz, in += rsiz) {
      for (i = 0; i < rsizw; i++)
        st[i] ^= ((uint64_t *) in)[i];
      f_traits::cryptonote_keccakf(st, CRYPTONOTE_KECCAK_ROUNDS);
    }
  ...
  int wild_cryptonote_keccak(const uint8_t *in, size_t inlen, uint8_t *md, size_t mdlen, callback_t cb)
  {
    state_t_m st;
    uint8_t temp[144];
    uint64_t rsiz, rsizw;
    ...
    for ( ; inlen >= rsiz; inlen -= rsiz, in += rsiz) 
    {
      ...
      for(size_t ll = 0; ll != CRYPTONOTE_KECCAK_ROUNDS; ll++)
      {
        if(ll != 0)
        {//skip first round
          mixin_t mix_in;
        ...
        f_traits::cryptonote_keccakf(st, 1);
      }
    }
  ...
  template<class f_traits, class callback_t>
  int wild_cryptonote_keccak_dbl(const uint8_t *in, size_t inlen, uint8_t *md, size_t mdlen, callback_t cb)
  {
    //Satoshi's classic
    wild_cryptonote_keccak<f_traits>(in, inlen, md, mdlen, cb);
    wild_cryptonote_keccak<f_traits>(md, mdlen, md, mdlen, cb);
    return 0;
  }
  ...
  {
  public:
    static void cryptonote_keccakf(uint64_t st[25], int rounds);
  };
