...
namespace crypto
{
  ...
  struct CLSAG_GG_signature
  {
    ...
    crypto::public_key    K1; // auxiliary key image for layer 1
  };
  ...
  {
    CLSAG_GG_input_ref_t(const crypto::public_key& stealth_address, const crypto::public_key& amount_commitment)
      : stealth_address(stealth_address), amount_commitment(amount_commitment)
    {}

    const crypto::public_key& stealth_address;   // P, not premultiplied by 1/8, TODO @#@#: make sure it's okay
    const crypto::public_key& amount_commitment; // A, premultiplied by 1/8
  };

  // pseudo_out_amount_commitment -- not premultiplied by 1/8
  bool generate_CLSAG_GG(const crypto::hash& m, const std::vector<CLSAG_GG_input_ref_t>& ring, const point_t& pseudo_out_amount_commitment, const key_image& ki,
  ...
  bool verify_CLSAG_GG(const crypto::hash& m, const std::vector<CLSAG_GG_input_ref_t>& ring, const crypto::public_key& pseudo_out_amount_commitment, const key_image& ki,
  ...
  struct CLSAG_GGX_signature
  {
    ...
    crypto::public_key    K1;   // auxiliary key image for layer 1 (G)
    crypto::public_key    K2;   // auxiliary key image for layer 2 (X)
  };

  struct CLSAG_GGX_input_ref_t : public CLSAG_GG_input_ref_t
  {
    CLSAG_GGX_input_ref_t(const crypto::public_key& stealth_address, const crypto::public_key& amount_commitment, const crypto::public_key& blinded_asset_id)
      : CLSAG_GG_input_ref_t(stealth_address, amount_commitment)
      , blinded_asset_id(blinded_asset_id)
    {}

    const crypto::public_key& blinded_asset_id; // T, premultiplied by 1/8
  };

  // pseudo_out_amount_commitment -- not premultiplied by 1/8
  // pseudo_out_asset_id         -- not premultiplied by 1/8
  bool generate_CLSAG_GGX(const crypto::hash& m, const std::vector<CLSAG_GGX_input_ref_t>& ring, const point_t& pseudo_out_amount_commitment, const point_t& pseudo_out_asset_id, const key_image& ki,
  ...
  bool verify_CLSAG_GGX(const crypto::hash& m, const std::vector<CLSAG_GGX_input_ref_t>& ring, const crypto::public_key& pseudo_out_amount_commitment,
    const crypto::public_key& pseudo_out_asset_id, const key_image& ki, const CLSAG_GGX_signature& sig);
  ...
  struct CLSAG_GGXG_signature
  {
    ...
    crypto::public_key    K1;   // auxiliary key image for layer 1 (G)
    crypto::public_key    K2;   // auxiliary key image for layer 2 (X)
    crypto::public_key    K3;   // auxiliary key image for layer 3 (G)
  };

  struct CLSAG_GGXG_input_ref_t : public CLSAG_GG_input_ref_t
  {
    CLSAG_GGXG_input_ref_t(const crypto::public_key& stealth_address, const crypto::public_key& amount_commitment, const crypto::public_key& concealing_point)
      : CLSAG_GG_input_ref_t(stealth_address, amount_commitment)
      , concealing_point(concealing_point)
    {}

    const crypto::public_key& concealing_point; // Q, premultiplied by 1/8
  };

  // pseudo_out_amount_commitment -- not premultiplied by 1/8
  // extended_amount_commitment   -- not premultiplied by 1/8
  bool generate_CLSAG_GGXG(const crypto::hash& m, const std::vector<CLSAG_GGXG_input_ref_t>& ring, const point_t& pseudo_out_amount_commitment, const point_t& extended_amount_commitment, const key_image& ki,
  ...
  // may throw an exception TODO @#@# make sure it's okay
  bool verify_CLSAG_GGXG(const crypto::hash& m, const std::vector<CLSAG_GGXG_input_ref_t>& ring, const crypto::public_key& pseudo_out_amount_commitment,
    const crypto::public_key& extended_amount_commitment, const key_image& ki, const CLSAG_GGXG_signature& sig);
  ...
  */
  ...
  struct CLSAG_GGXXG_signature
  {
    ...
    crypto::public_key    K1;   // auxiliary key image for layer 1 (G)
    crypto::public_key    K2;   // auxiliary key image for layer 2 (X)
    crypto::public_key    K3;   // auxiliary key image for layer 2 (X)
    crypto::public_key    K4;   // auxiliary key image for layer 3 (G)
  };

  struct CLSAG_GGXXG_input_ref_t : public CLSAG_GGX_input_ref_t
  {
    CLSAG_GGXXG_input_ref_t(const crypto::public_key& stealth_address, const crypto::public_key& amount_commitment, const crypto::public_key& blinded_asset_id, const crypto::public_key& concealing_point)
  ...
  // pseudo_out_asset_id          -- not premultiplied by 1/8
  // extended_amount_commitment   -- not premultiplied by 1/8
  bool generate_CLSAG_GGXXG(const crypto::hash& m, const std::vector<CLSAG_GGXXG_input_ref_t>& ring, const point_t& pseudo_out_amount_commitment, const point_t& pseudo_out_blinded_asset_id, const point_t& extended_amount_commitment, const key_image& ki,
  ...
  // extended_amount_commitment   -- premultiplied by 1/8
  // may throw an exception TODO @#@# make sure it's okay
  bool verify_CLSAG_GGXXG(const crypto::hash& m, const std::vector<CLSAG_GGXXG_input_ref_t>& ring, const crypto::public_key& pseudo_out_amount_commitment, const crypto::public_key& pseudo_out_blinded_asset_id, const crypto::public_key& extended_amount_commitment, const key_image& ki,
